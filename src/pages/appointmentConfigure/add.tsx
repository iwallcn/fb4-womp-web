...
import { Grid, Field, Form, TreeSelect, Input, Select, Table, DatePicker, TimePicker, Message, NumberPicker, Button } from '@alifd/next';
      ...
      if (res.success) {
        ...
        if (dis && dis.length) {
          for (let i = 0; i < dis.length; i++) {
            let obj = {
              // [`beginTime${i}`]: [moment(dis[i].beginTime * 1000), moment(dis[i].endTime * 1000)]
              [`beginTime${i}`]: [moment(dis[i].beginTime), moment(dis[i].endTime)]
            }
            hList.push({ beginTime: '', id: new Date().getTime() });
            searchField.setValues(obj);
          }
          setHlist([...hList]);
        }
      } else {
  ...
  const submit = () => {
    searchField.validate((errors, values: any) => {
      ...
      for (let i = 0; i < hList.length; i++) {
        let t = searchField.getValue(`beginTime${i}`);
        if (t) {
          let obj = {
    ...
    Object.keys(hList).map((v, i) => {
      ...
      return (
        <div className={styles.holiday} key={id}>
          <FormItem key={id}>
            <DatePicker.RangePicker disabledDate={disabledDate} name={`beginTime${i}`} format="YYYY-MM-DD" key={id} />
            <CustomIcon type="close" className={styles.news} onClick={() => handleDeleteH(i)} />
          </FormItem>
        </div>
      )
    })
  ...
  const FormPage = (
    <div className="Search">
      <Form field={searchField} labelAlign="left" {...formLayout}>
        ...
        <h5 className="Title_h5">{lang['fb4.cannot.appointment.date.set']} <CustomIcon type="news" onClick={handleAddH} className={styles.news} /> </h5>
